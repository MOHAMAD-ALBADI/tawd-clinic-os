...
import type { getUserClaims } from "@/lib/auth/get-user-claims";

/* What the bell knows.
 *
 * This used to live inside the notifications route, which meant it was
 * reachable by exactly one caller: the bell in the top bar. Sura could
 * not see it.
 *
 * So when the screen said «١٦ فاتورة متأخرة تحتاج متابعة» and the owner
 * asked her to deal with it, she queried sura_alerts — a table of
 * emergencies and complaints, genuinely empty — and answered that there
 * were no alerts at all. She was not refusing. She could not see the
 * screen she was being asked about.
 *
 * Every line here is computed from the clinic's own rows, so it is
 * equally correct for a clinic whose data nobody has seen yet. That is
 * the point: the agent reads the live view, not a list somebody wrote
 * down for it.
 */
...
type Claims = NonNullable<Awaited<ReturnType<typeof getUserClaims>>>;

export async function notificationsFor(claims: Claims): Promise<NotifItem[]> {
