...
const STATUS: Record<string, { label: string; color: string }> = {
  scheduled: { label: "مجدول", color: "#94A3B8" }, confirmed: { label: "مؤكد", color: "var(--accent-1)" },
...
export default async function PatientFilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const vitalCard = (label: string, val: unknown, unit = "") =>
    val !== null && val !== undefined ? (
      <div className="rounded-xl px-3 py-2.5" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <div className="text-base font-bold text-white ltr-nums">{String(val)}<span className="text-[10px] font-normal mr-1" style={{ color: "var(--text-3)" }}>{unit}</span></div>
      </div>
    ) : null;
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <Link href="/doctor/patients" className="inline-flex items-center gap-1.5 text-xs" style={{ color: "var(--text-3)" }}>
        <ArrowRight className="w-3.5 h-3.5" /> رجوع لمرضاي
      </Link>

      {/* Header */}
      <div className="rounded-2xl p-5 flex items-center gap-4" style={{ background: "linear-gradient(135deg, rgb(var(--accent-2-rgb) / 0.1) 0%, rgba(6,14,30,0.92) 60%)", border: "1px solid rgb(var(--accent-1-rgb) / 0.2)" }}>
        <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-white text-xl font-black shrink-0" style={{ background: "linear-gradient(135deg,var(--accent-2),var(--accent-2))" }}>
      ...
      {latest && (
        <div className="rounded-2xl p-5" style={{ background: "rgba(6,14,30,0.85)", border: "1px solid rgba(255,255,255,0.07)" }}>
          <h3 className="font-bold text-white mb-3 flex items-center gap-2 text-sm"><Activity className="w-4 h-4" style={{ color: "var(--accent-1)" }} /> أحدث العلامات الحيوية <span className="text-[10px] font-normal" style={{ color: "var(--text-3)" }}>· {dt(latest.recorded_at)}</span></h3>
      ...
      <div className="rounded-2xl p-5" style={{ background: "rgba(6,14,30,0.85)", border: "1px solid rgba(255,255,255,0.07)" }}>
        <h3 className="font-bold text-white mb-4 flex items-center gap-2 text-sm"><NotebookPen className="w-4 h-4" style={{ color: "var(--accent-1)" }} /> الملاحظات السريرية</h3>
        {noteList.length === 0 ? (
          <p className="text-xs text-center py-6" style={{ color: "var(--text-3)" }}>لا ملاحظات بعد</p>
        ) : (
          <div className="space-y-3">
            {noteList.map((n) => (
              <div key={n.id} className="rounded-xl p-3.5" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
                ...
                <div className="flex items-center gap-2 mt-2 text-[11px]" style={{ color: "var(--text-3)" }}>
      ...
      {/* Appointment history */}
      <div className="rounded-2xl p-5" style={{ background: "rgba(6,14,30,0.85)", border: "1px solid rgba(255,255,255,0.07)" }}>
        <h3 className="font-bold text-white mb-4 flex items-center gap-2 text-sm"><CalendarDays className="w-4 h-4" style={{ color: "var(--accent-1)" }} /> سجل المواعيد</h3>
        {apptList.length === 0 ? (
          <p className="text-xs text-center py-6" style={{ color: "var(--text-3)" }}>لا مواعيد</p>
        ) : (
            ...
            {apptList.map((ap) => {
              ...
              return (
                <div key={ap.id} className="flex items-center gap-3 px-3 py-2 rounded-lg" style={{ background: "rgba(255,255,255,0.02)" }}>
                  <span className="text-xs ltr-nums shrink-0" style={{ color: "var(--text-3)" }}>{d(ap.slot_time)}</span>
