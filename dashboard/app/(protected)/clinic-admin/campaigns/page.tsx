...
import { Radio, Plus, CheckCircle2, Clock, AlertCircle } from "lucide-react";

export const metadata = { title: "الحملات التسويقية — طود" };
...
export default async function CampaignsPage() {
  ...
  const { data, count } = await sb
    ...
    .select(
      "id, name, template_name, status, total_recipients, sent_count, " +
      "failed_count, scheduled_at, started_at, completed_at, created_at",
      { count: "exact" }
    )
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: "rgba(148,163,184,0.55)" }}>
            إرسال رسائل واتساب لشرائح المرضى
          </p>
        </div>
        <button
          disabled
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold opacity-50 cursor-not-allowed"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "إجمالي الحملات",  value: count ?? 0,  color: "#818CF8", glow: "rgba(99,102,241,0.07)",  border: "rgba(99,102,241,0.18)",  Icon: Radio       },
          { label: "تعمل الآن",        value: running,     color: "#38bdf8", glow: "rgba(56,189,248,0.07)",  border: "rgba(56,189,248,0.18)",  Icon: Clock       },
          { label: "مكتملة",          value: completed,   color: "#4ADE80", glow: "rgba(34,197,94,0.07)",   border: "rgba(34,197,94,0.18)",   Icon: CheckCircle2 },
          { label: "إجمالي المُرسَل", value: totalSent.toLocaleString("ar-SA"), color: "#5dd9cb", glow: "rgba(94,217,203,0.07)", border: "rgba(94,217,203,0.18)", Icon: AlertCircle },
        ].map((s) => (
          <div
            key={s.label}
          ...
          >
            ...
            >
              {typeof s.value === "number" ? s.value : "٠"}
            </div>
            <div className="relative flex items-center justify-between mb-3">
              <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "rgba(148,163,184,0.45)" }}>
                {s.label}
              </p>
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-sm mt-1" style={{ color: "#334155" }}>
                الحملات تُطلق عبر n8n أو من لوحة التحكم
              </p>
            </div>
          </div>
        ) : (
