// Real availability for Sura's booking: overlap-aware check (not just same-minute) + suggest
// actually-free times that day when the requested slot is taken. Keeps waitlist-fill recovery.
...
const openTs  = dateOk ? (c.b_date + 'T' + pad(openH) + ':00:00+04:00') : '';
const closeTs = dateOk ? (c.b_date + 'T' + pad(closeH) + ':00:00+04:00') : '';
const dayStart = dateOk ? (c.b_date + 'T00:00:00+04:00') : '';
const dayEnd   = dateOk ? (c.b_date + 'T23:59:59+04:00') : '';
const valid = !!(svc && svc.id && slot_time && future && c.clinic_id && c.patient_id && dateOk);
const IV = dur + " || ' minutes'";  // -> '30 minutes'
let sql;
if (valid) {
  sql =
    "WITH ex AS (SELECT slot_time, duration_minutes FROM public.appointments WHERE doctor_id='" + DOCTOR_ID + "'::uuid AND clinic_id='" + c.clinic_id + "'::uuid AND deleted_at IS NULL AND status NOT IN ('cancelled','no_show') AND slot_time >= '" + dayStart + "'::timestamptz AND slot_time < '" + dayEnd + "'::timestamptz), " +
    ...
    "upd AS (UPDATE public.patients SET name='" + name + "', updated_at=now() WHERE id='" + c.patient_id + "'::uuid AND '" + name + "'<>'' AND (name IS NULL OR btrim(name)='' OR name ILIKE '%واتساب%' OR name ILIKE '%whatsapp%' OR name ILIKE '%مريض%') AND (SELECT ok FROM reqchk) RETURNING 1), " +
    "ins AS (INSERT INTO public.appointments (clinic_id,patient_id,doctor_id,service_id,slot_time,duration_minutes,status,type,notes) SELECT '" + c.clinic_id + "'::uuid,'" + c.patient_id + "'::uuid,'" + DOCTOR_ID + "'::uuid,'" + svc.id + "'::uuid,'" + slot_time + "'::timestamptz," + dur + ",'scheduled','consultation','حجز عبر سُرى (واتساب)' WHERE (SELECT ok FROM reqchk) RETURNING id), " +
    ...
    "cand AS (SELECT gs AS slot FROM generate_series('" + openTs + "'::timestamptz, '" + closeTs + "'::timestamptz - (" + IV + ")::interval, (" + IV + ")::interval) gs WHERE gs > now() AND NOT EXISTS (SELECT 1 FROM ex WHERE ex.slot_time < gs + (" + IV + ")::interval AND ex.slot_time + (ex.duration_minutes||' minutes')::interval > gs) ORDER BY gs LIMIT 4) " +
    "SELECT CASE WHEN EXISTS(SELECT 1 FROM ins) THEN 'confirmed' WHEN (SELECT ok FROM reqchk)=false THEN 'unavailable' ELSE 'error' END AS booking_status, (SELECT id::text FROM ins) AS appointment_id, (SELECT string_agg(to_char(slot AT TIME ZONE 'Asia/Muscat','HH24:MI'), ', ' ORDER BY slot) FROM cand) AS alternatives;";
} else {
  sql = "SELECT 'error' AS booking_status, NULL::text AS appointment_id, NULL::text AS alternatives;";
}
return [{ json: {
  sql, service_name_ar: svc ? svc.name_ar : (c.b_service_ar||''), price: svc ? svc.price : '',
  slot_time, patient_name: c.b_patient_name || '', clinic_id: c.clinic_id, patient_id: c.patient_id,
  doctor_id: DOCTOR_ID, _parent_ctx: c
}}];`;
...
const d = prep.slot_time ? new Date(prep.slot_time) : null;
...
if (r.booking_status === 'confirmed') {
  ...
  msg = 'للأسف ' + timeAr + ' محجوز ذاك اليوم 🌿' +
        (r.alternatives ? ('\nأقرب الأوقات المتاحة: ' + r.alternatives + '\nأي وقت يناسبك؟') : '\nتحب أقترح لك يوماً أو وقتاً ثانياً؟');
} else {
  msg = 'تمام 🌿 ممكن تعطيني اليوم والوقت المناسب عشان أثبّت الموعد؟';
}
...
(async () => {
  ...
  console.log("✓ Real availability (overlap check + alternatives) wired into booking");
