  ...
  return (
    <>
      <div className="relative">
        <Search className="w-4 h-4 absolute top-1/2 -translate-y-1/2 start-3" style={{ color: "var(--text-3)" }} />
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(6,14,30,0.85)", border: "1px solid rgba(255,255,255,0.07)" }}>
        {filtered.length === 0 ? (
          <p className="text-sm text-center py-12" style={{ color: "var(--text-3)" }}>لا نتائج</p>
        ) : (
          filtered.map((p, i) => (
            <Link key={p.id} href={`/doctor/patients/${p.id}`} className="flex items-center gap-4 px-4 py-3 transition-colors hover:bg-white/[0.03]"
              ...
              <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold shrink-0" style={{ background: "linear-gradient(135deg,var(--accent-2),var(--accent-2))" }}>{p.name.charAt(0)}</div>
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-sm text-white truncate">{p.name}</div>
                {p.phone && <div className="text-xs flex items-center gap-1 ltr-nums" style={{ color: "var(--text-3)" }}><Phone className="w-3 h-3" /> {p.phone}</div>}
              </div>
              <div className="text-center shrink-0">
                <div className="text-sm font-bold" style={{ color: "var(--accent-1)" }}>{p.visits}</div>
                <div className="text-[10px]" style={{ color: "var(--text-3)" }}>زيارة</div>
              </div>
              <div className="text-end shrink-0 hidden sm:block">
                <div className="text-[11px]" style={{ color: "var(--text-3)" }}>آخر زيارة</div>
